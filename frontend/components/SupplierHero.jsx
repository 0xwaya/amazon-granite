    ...
    <section className="bg-surface border border-border rounded-2xl p-8 mb-10">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-wrap gap-3 mt-3 text-sm">
            ...
            {onToggleGallery && (
              ...
              >
                {showGallery ? 'Hide featured slabs' : isLoadingGallery ? 'Loading featured slabs...' : `Load ${supplier.featuredCount || 'featured'} slabs`}
              </button>
            )}
          </div>
        </div>
      ...
      {!showGallery ? (
        <div className="mt-8 rounded-xl border border-border bg-panel px-4 py-3 text-sm text-muted">
          Featured slab imagery loads on demand to keep the homepage fast.
        </div>
      ) : null}
      {showGallery && isLoadingGallery ? (
        <div className="mt-8 rounded-xl border border-border bg-panel px-4 py-3 text-sm text-muted">
          Loading featured slabs...
        </div>
      ) : null}
